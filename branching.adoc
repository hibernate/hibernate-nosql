...
The GHA (GitHub Action) workflow, that is checked into the repository at `.github/workflows/ci.yml`,
refers to the branch name it applies to, which is typically `main`.
When branching off for a new major/minor release, the reference to the branch name needs to be updated,
in order for workflows to run again on pull requests against that branch.

At the very top of the file in the section `on > push > branches` and `on > pull_request > branches`,
replace the `main` branch name with the correct branch name.